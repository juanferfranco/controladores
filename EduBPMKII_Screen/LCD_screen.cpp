...
void LCD_screen::dBitmap(uint16_t x, uint16_t y, const uint16_t *image, int16_t w, int16_t h){
 int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  ...
  int i = w*(h - 1);
  uint16_t _height = screenSizeY();
  uint16_t _width = screenSizeX();
  ...
  _setWindow(x, y-h+1, x+w-1, y);
  
  
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
      _writeData88((uint8_t)(image[i] >> 8), (uint8_t)image[i]);
...
void LCD_screen::dBitmap8(uint16_t x, uint16_t y, const uint8_t *image, int16_t w, int16_t h){
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  uint16_t _height = screenSizeY();
  uint16_t _width = screenSizeX();
