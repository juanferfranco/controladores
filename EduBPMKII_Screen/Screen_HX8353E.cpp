...
void Screen_HX8353E::_writeData16(uint16_t data16)
{
    ...
    digitalWrite(_pinDataCommand, HIGH);
    digitalWrite(_pinChipSelect, LOW);
    buffer[0] = highByte(data16);
    buffer[1] = lowByte(data16);
...
void Screen_HX8353E::_writeData88(uint8_t dataHigh8, uint8_t dataLow8)
{
    ...
    //digitalWrite(_pinDataCommand, HIGH);
    //digitalWrite(_pinChipSelect, LOW);
    GPIO_write(_pinDataCommand,1);
    GPIO_write(_pinChipSelect,0);
    ...
    GPIO_write(_pinChipSelect,1);
    
    //digitalWrite(_pinChipSelect, HIGH);
}

void Screen_HX8353E::_writebitMap(const uint8_t *image, int16_t size){
    uint16_t newSize = size * 2;
    ...
    if(newSize > 1023){
        framesNumber = newSize/1023;
        for(;i<framesNumber;i++){
            SPI.transfer((uint8_t *)(image+1023*i),1023);    
        }
        SPI.transfer((uint8_t *)(image+1023*i),newSize % 1023);
    }
    else{
        SPI.transfer((uint8_t *)image,newSize);
    }
